...
反応:
  ...
  RWGS: CO2 + H2  ⇌ CO + H2O      (逆水性ガスシフト。network の量論もこの向き)
...
#   ★再現検証★ examples/ortega_zsm5.py（Fig.4 の T×p_M グリッド 15 点・Fig.6/7 のアレニウス）。
#     デジタイズ不要の厳密アンカーが SI Table S6/S7 にある: 190℃・p_MeOH=0.931 bar・乾燥・
#     WHSV 100 h⁻¹ で 89.9 mol·m⁻³cat·s⁻¹ ÷ 床密度 1300 kg·m⁻³ = 0.069154 mol_MeOH·kg⁻¹·s⁻¹
#     → 本実装 0.0683（−1.2%）。152℃以上のグリッドは平均 2%・最大 5% で一致する。
#   ⚠️ 原著の装置は無勾配循環反応器なので、再現には PFR ではなく reactors.cstr を使うこと
#     （触媒が見るのは出口組成。190℃ は転化率 8% あり、無視すると 4% ずれる）。
...
#   ⚠️ K_eq2 は**原著の逆数**を保持する: 原著(Graaf1986 eq12)の RWGS 平衡定数は
#      log10 K_eq,2 = −2073/T + 2.029（CO2+H2⇌CO+H2O）。Ng eq(9) は駆動力に (1/K_eq,2) を
#      掛けるので、本コードの K_eq2 ≡ 1/K_eq,2 = 10^(2073/T − 2.029) を保持し、そのまま掛ける。
#      （523 K で K_eq2=86.1、すなわち K_RWGS=0.0116。文献値 ~0.012 と一致）
EQ_KOGAS = {
    "K_eq1": (3066.0, -10.592),
